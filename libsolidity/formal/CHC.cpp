...
void CHC::checkAssertTarget(ASTNode const* _scope, CHCVerificationTarget const& _target)
{
	...
	{
		createErrorBlock();
		connectBlocks(_target.value, error(), _target.constraints && (_target.errorId == static_cast<size_t>(assertion->id())));
